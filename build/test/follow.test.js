...
export const follow = request => {
    ...
    it('authentication user two', async done => {
        ...
        request.post('/graphql').set('Content-Type', 'application/json').set('Accept', '*/*').send({ query }).then(query => {
    ...
    it('user on follow user2', async done => {
        ...
        request.post('/graphql').set('Content-Type', 'application/json').set('token', IntegTestDataUserOne.token).set('Accept', '*/*').send({ query }).then(response => {
            let res = JSON.parse(response.text);
            expect(response.status).toBe(200);
            console.log(res);
    ...
    it('userTwo follow userOne', async done => {
        ...
        request.post('/graphql').set('Content-Type', 'application/json').set('token', IntegTestDataUserTwo.token).set('Accept', '*/*').send({ query }).then(response => {
            let res = JSON.parse(response.text);
            expect(response.status).toBe(200);
